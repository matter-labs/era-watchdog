...
const withLatency = async <T>(fn: () => Promise<T>): Promise<{ return: T; latency: number }> => {
...
class SimpleTxFlow {
    ...
    protected async populateTx(request: types.TransactionRequest): Promise<types.TransactionRequest> {
        return this.wallet.populateTransaction(request);
    }

    protected async sendTx(populated: types.TransactionRequest): Promise<types.TransactionResponse> {
        return this.wallet.sendTransaction(populated);
    }
    ...
    protected async step() {
        ...
        try {
            ...
            const { return: populated, latency: estimate_letancy } = await withLatency(() => this.populateTx(tx));
            ...
            // send transaction
            const { return: txResponse, latency: send_latency } = await withLatency(() => this.sendTx(populated));
            send_success = 1;
